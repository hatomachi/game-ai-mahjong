...
import { User, Cpu, Flame, Layers } from 'lucide-react';
...
interface MahjongTableProps {
  ...
  onResetGame: () => void;
  onNextRound: () => void;
  onPlayerDiscard: (tileId: string, isTsumo: boolean) => void;
...
export const MahjongTable: React.FC<MahjongTableProps> = ({
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950 text-slate-100 p-3 space-y-3 overflow-y-auto">
      {/* 1. 上部コントロール & 局情報バー */}
      ...
      {/* 2. 四方麻雀卓グラフィック */}
      <div className="relative flex-1 min-h-[420px] rounded-2xl bg-gradient-to-b from-[#184427] to-[#102e1a] border-4 border-amber-950 shadow-inner p-3 flex flex-col justify-between">
        {/* (A) 対面 (CPU-2) - 上側 */}
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="text-xs font-bold text-emerald-100 flex items-center gap-1.5">
                ...
                {toimen.isRiichi && (
                  <span className="bg-rose-600 text-white text-[8px] px-1 rounded animate-pulse">
                    立直
                  </span>
                )}
              </div>
