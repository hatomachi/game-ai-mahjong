...
export const CenterSquare: React.FC<CenterSquareProps> = ({ gameState }) => {
  ...
  return (
    <div className="relative w-full max-w-[280px] bg-gradient-to-b from-slate-900 via-slate-950 to-slate-900 rounded-2xl border-2 border-emerald-600/60 shadow-2xl p-2.5 flex flex-col items-center justify-between gap-2 select-none">
      {/* 4方向の手番点灯インジケーター（東西南北） */}
      {/* 奥: 対面 (player 2) */}
      <div className="absolute top-1 left-1/2 -translate-x-1/2 flex items-center gap-1 text-[9px] font-bold">
        <span
          className={`px-1.5 py-0.2 rounded transition-all ${
            activePlayerIndex === 2
              ? 'bg-amber-400 text-slate-950 ring-2 ring-amber-300 animate-pulse font-black scale-105'
      ...
      {/* 左: 上家 (player 3) */}
      <div className="absolute left-1.5 top-1/2 -translate-y-1/2 flex items-center gap-1 text-[9px] font-bold">
        <span
          className={`px-1.5 py-0.2 rounded transition-all ${
            activePlayerIndex === 3
              ? 'bg-amber-400 text-slate-950 ring-2 ring-amber-300 animate-pulse font-black scale-105'
      ...
      {/* 右: 下家 (player 1) */}
      <div className="absolute right-1.5 top-1/2 -translate-y-1/2 flex items-center gap-1 text-[9px] font-bold">
        <span
          className={`px-1.5 py-0.2 rounded transition-all ${
            activePlayerIndex === 1
              ? 'bg-amber-400 text-slate-950 ring-2 ring-amber-300 animate-pulse font-black scale-105'
      ...
      {/* 手前: 自家 (player 0) */}
      <div className="absolute bottom-1 left-1/2 -translate-x-1/2 flex items-center gap-1 text-[9px] font-bold">
        <span
          className={`px-1.5 py-0.2 rounded transition-all ${
            activePlayerIndex === 0
              ? 'bg-amber-400 text-slate-950 ring-2 ring-amber-300 animate-pulse font-black scale-105'
      ...
      <div className="mt-3 text-center">
        <div className="text-sm font-black text-amber-400 flex items-center justify-center gap-1 drop-shadow">
        ...
        <div className="text-[10px] text-slate-400 flex items-center justify-center gap-2 mt-0.5">
      ...
      {/* 3. 供託エリア（リーチ棒・本場棒） */}
      <div className="w-full flex flex-col items-center justify-center gap-1 bg-slate-950/90 py-1.5 px-2 rounded-lg border border-slate-800">
        <div className="flex items-center justify-around w-full gap-2 min-h-[22px]">
          {/* リーチ棒（供託） */}
          {riichiSticks > 0 ? (
            ...
            <span className="text-[9px] text-slate-600">供託なし</span>
          )}
        ...
        {/* 直前の捨て牌プレビュー */}
        {lastDiscard && (
          <div className="flex items-center gap-1 text-[10px] text-slate-400 border-t border-slate-800/80 pt-1 w-full justify-center">
            <span>直前打牌:</span>
      ...
      {gameState.roundResult && (
        <div className="mb-3 px-2 py-0.5 bg-rose-600/30 border border-rose-500 text-rose-200 text-[10px] font-bold rounded animate-pulse">
